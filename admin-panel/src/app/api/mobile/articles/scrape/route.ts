...
// Mobile scrape endpoint - no auth required for mobile app
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        console.log('[Mobile Scrape] Fetching URL:', url);
        ...
        if (!response.ok) {
            return NextResponse.json({ error: `Failed to fetch URL: ${response.statusText}` }, { status: 400, headers: corsHeaders });
        }
        ...
        console.log('[Mobile Scrape] Extracted article text, length:', articleText.length);
        ...
        if (articleText && articleText.length > 50) {
            try {
                ...
                console.log('[Mobile Scrape] Successfully generated paraphrased summary');
            } catch (error) {
                console.error('[Mobile Scrape] Failed to generate paraphrased summary, using original:', error);
            }
        } else {
            console.log('[Mobile Scrape] Article text too short, skipping AI paraphrasing');
        }
        ...
        console.log('[Mobile Scrape] Successfully scraped article:', cleanedTitle);

        return NextResponse.json({ article: scrapedData }, { headers: corsHeaders });
    } catch (error) {
        console.error('[Mobile Scrape] Scrape error:', error);
        return NextResponse.json({ error: 'Failed to scrape article' }, { status: 500, headers: corsHeaders });
    }
}
